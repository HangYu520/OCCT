...
计算基函数的导数对于求曲线切矢量、曲面法矢量以及进行各种几何分析至关重要。

#### 1. 基函数的导数

$p$ 次 B 样条基函数 $N_{i,p}(u)$ 的导数可以通过低一阶的基函数线性组合表示：
...
除了上述基本性质，B 样条基函数还具有一些支撑 CAD/CAE 理论的核心高级性质：

*   **线性无关性 (Linear Independence)：** 在给定的节点矢量 $U$ 上，基函数集合 $\{N_{i,p}(u)\}$ 是线性无关的。这意味着：
    $$ \sum_{i} c_i N_{i,p}(u) = 0 \iff c_i = 0, \forall i $$
    这一性质保证了控制点与曲线形状之间的一一对应关系，是**曲线拟合 (Curve Fitting)** 和**插值 (Interpolation)** 具有唯一解的基础。

*   **变差缩减性 (Variation Diminishing Property)：** 这是一个极其重要的几何性质。它指出，任意平面与 B 样条曲线的交点个数，不会超过该平面与该曲线控制多边形的交点个数。
    - **物理意义：** B 样条曲线比它的控制多边形更平滑，它不会产生控制多边形之外的额外波动。

*   **积分性质 (Integration)：** $p$ 次 B 样条基函数的定积分可以通过 $p+1$ 次基函数表示：
    $$ \int_{u_i}^{u_m} N_{i,p}(u) du = \frac{u_{i+p+1} - u_i}{p+1} $$
    这在计算曲线长度、面积以及质量属性时非常有用。

*   **嵌套空间与完备性 (Nested Spaces)：** 如果我们通过**节点插入 (Knot Insertion)** 在节点矢量中增加新的节点，那么旧节点矢量生成的函数空间是新节点矢量生成的函数空间的子集。
    $$ \mathcal{S}_{p, U} \subset \mathcal{S}_{p, U \cup \{u_{new}\}} $$
    这意味着可以在不改变曲线几何形状的前提下，通过增加节点来提升曲线的自由度（即增加控制点），从而进行更精细的局部调整。
