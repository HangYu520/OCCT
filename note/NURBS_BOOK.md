...
#### 1.3.3. 齐次坐标 (Homogeneous Coordinates)
...
#### 2.1.2. B 样条基函数的定义 (Cox-de Boor 递归公式)
...
B 样条曲线（B‑Spline Curve）是 CAD 领域最常用的一种曲线表示方法。它由 B 样条基函数与控制点线性组合而成，具有**局部支撑性**、**凸包性**以及灵活的**连续性控制**等优良性质，非常适合几何建模和交互设计。

#### 3.1.1 定义

给定：
- **控制点** $\{\mathbf{P}_i\}_{i=0}^n$，每个 $\mathbf{P}_i \in \mathbb{R}^d$（通常 $d=2$ 或 $3$）。
- **节点矢量 (Knot Vector)** $U = \{u_0, u_1, \dots, u_m\}$，满足 $u_i \le u_{i+1}$。
- **次数 (Degree)** $p$（又称阶数 Order = $p+1$）。

**$p$ 次 B 样条曲线**的定义为：
$$
\mathbf{C}(u) = \sum_{i=0}^n N_{i,p}(u) \,\mathbf{P}_i, \quad u \in [u_p, u_{m-p}]
$$
其中 $N_{i,p}(u)$ 是由节点矢量 $U$ 与次数 $p$ 决定的第 $i$ 个 $p$ 次 B 样条基函数（参见 §2.1）。

**参数定义域**：通常取 $u \in [u_p, u_{m-p}]$，以保证在该区间内基函数满足**单位分解性**，且曲线具备完整性质。若节点矢量是**准均匀 (Clamped/Open)** 的（即两端节点重复 $p+1$ 次），则曲线经过第一个与最后一个控制点，定义域为 $[u_0, u_m]$。

> **重要关系**：节点个数 $m+1$、控制点个数 $n+1$ 与次数 $p$ 满足：
> $$ m = n + p + 1 $$

#### 3.1.2 重要性质

B 样条曲线继承了基函数的几何与代数性质，这些性质是其成为 CAD 标准表示的关键。

- **局部支撑性 (Local Support)**：由于 $N_{i,p}(u)$ 仅在区间 $[u_i, u_{i+p+1})$ 非零，因此控制点 $\mathbf{P}_i$ 只影响该区间对应的曲线段。这一性质使得**局部编辑**成为可能——修改一个控制点只会改变曲线的一部分形状。

- **凸包性 (Convex Hull Property)**：对于任一参数 $u \in [u_j, u_{j+1})$，曲线点 $\mathbf{C}(u)$ 位于由该区间内非零基函数对应的控制点构成的凸包中。进一步地，若节点区间 $[u_j, u_{j+1})$ 内非零的基函数个数为 $p+1$，则曲线段位于这 $p+1$ 个控制点的凸包内。

- **强凸包性 (Strong Convex Hull Property)**（针对准均匀 B 样条）：若节点矢量两端节点重复 $p+1$ 次，则整个曲线完全位于所有控制点的凸包内。

- **变差缩减性 (Variation Diminishing Property)**：任意平面（或直线）与 B 样条曲线的交点个数，不会超过该平面（或直线）与曲线控制多边形的交点个数。这一性质保证了曲线不会出现比控制多边形更多的“波动”，从而具有更好的光顺性。

- **连续性 (Continuity)**：在节点内部，曲线是无限次可微的（$C^\infty$）。在节点 $u_i$ 处，若其重数为 $k$（$1 \le k \le p$），则曲线在该节点处具有 $C^{p-k}$ 连续性。
  - 若 $k = p$，曲线在该节点处仅为 $C^0$ 连续（位置连续，但切矢可能不连续）。
  - 若 $k = p+1$，曲线在该节点处可能出现间断（通常用于构造非闭合曲线的端点）。

- **仿射不变性 (Affine Invariance)**：对控制点进行仿射变换（平移、旋转、缩放、剪切）等价于对曲线进行同样的变换。这一性质使得曲线在几何变换下保持形状不变。

- **分段多项式性 (Piecewise Polynomial)**：B 样条曲线在每个节点区间 $[u_i, u_{i+1})$ 上是一个 $p$ 次多项式曲线。因此，整条曲线是由若干段多项式曲线“拼接”而成的。

#### 3.1.3 曲线求值：de Boor 算法

计算 B 样条曲线上一点 $\mathbf{C}(u)$ 的高效、稳定的算法是 **de Boor 算法**。它是 De Casteljau 算法在 B 样条上的推广。

**算法步骤**：
设 $u \in [u_j, u_{j+1})$，且 $u$ 不在节点上（若 $u$ 为节点，可通过节点重数处理）。

1. **初始化**：
   $$ \mathbf{P}_i^0 = \mathbf{P}_i, \quad i = j-p, j-p+1, \dots, j $$

2. **递归计算**（$r = 1, \dots, p$）：
   $$ \mathbf{P}_i^r = (1-\alpha_i^r) \mathbf{P}_{i-1}^{r-1} + \alpha_i^r \mathbf{P}_i^{r-1} $$
   其中插值系数 $\alpha_i^r$ 为：
   $$ \alpha_i^r = \frac{u - u_i}{u_{i+p-r+1} - u_i} $$

3. **得到曲线点**：
   $$ \mathbf{C}(u) = \mathbf{P}_j^p $$

**几何意义**：de Boor 算法通过反复对控制多边形（及其迭代中间点）进行线性插值，最终收敛到曲线上的点。该过程还同时给出了曲线在 $u$ 处**分割**后的两段子曲线的控制点，可用于曲线的细分（Subdivision）。

#### 3.1.4 特殊节点矢量类型

根据节点矢量的分布，B 样条曲线可分为以下几类：

- **均匀 B 样条 (Uniform B‑Spline)**：节点间距为常数，即 $u_{i+1} - u_i = \text{const}$。此时基函数 $N_{i,p}(u)$ 在所有区间上具有相同的形状，仅做平移。均匀 B 样条计算简单，但曲线一般不经过控制点。

- **准均匀 B 样条 (Clamped/Open B‑Spline)**：两端节点重复 $p+1$ 次，内部节点均匀或非均匀分布。这是 CAD 中最常用的形式，因为它保证了曲线**经过首末控制点**，且具有完整的凸包性。例如对于 $p=3$，节点矢量可为：
  $$ \{0,0,0,0, 0.25,0.5,0.75, 1,1,1,1\} $$

- **分段贝塞尔形式 (Piecewise Bézier Form)**：若每个内部节点的重数均为 $p$，则 B 样条曲线退化为一系列 $p$ 次贝塞尔曲线段，在各段连接处仅 $C^0$ 连续。通过调整节点重数，可以在同一表示中混合不同连续性的曲线段。

- **非均匀 B 样条 (Non‑Uniform B‑Spline)**：节点间距任意，且允许重节点。这是最一般的形式，提供了最大的灵活性，能够通过节点重数精确控制连续性。

B 样条曲线是贝塞尔曲线的推广：
- 当节点矢量取为 $\{0,\dots,0,1,\dots,1\}$（两端节点重数均为 $p+1$，且无内部节点）时，B 样条曲线退化为一条 $p$ 次贝塞尔曲线。
- 通过节点插入，可以将 B 样条曲线转换为一系列首尾相连的贝塞尔曲线段（每个节点区间对应一段贝塞尔曲线）。

这种统一性使得 B 样条既能保留贝塞尔曲线的优良性质，又克服了其全局性、次数随控制点增加而升高的缺点。